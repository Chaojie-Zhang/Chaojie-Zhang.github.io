...
| 符号                        | 含义                   | 决定什么           |
| --------------------------- | ---------------------- | ------------------ |
| \\(N\sb {\mathrm{vocab}}\\) | 词表里有多少种 token   | embedding 表的行数 |
...
这里需要区分两类量：**参数是模型保存的可训练数值；状态是给定输入后计算出的中间结果。** \\(E\\) 是参数矩阵，查表得到的 \\(X\sb {\mathrm{token}}\\) 是当前输入的表示。
...
“某个位置的 query”指 \\(Q\\) 中对应的那一行，是一个向量。同一个头中，所有位置都使用同一套 \\(W\sb {Q}\\)。矩阵乘法将每一行分别投影，再把结果按行排列；每个位置并没有独立的投影参数。\\(K\\) 和 \\(V\\) 同理。
...
设八个头的输出为 \\(H\sb {1},\ldots,H\sb {8}\\)。沿列方向拼接，再经过注意力输出投影：
...
拼接结果和 \\(O\\) 的尺寸都是 \\(S\times512\\)。\\(W\sb {O}\\) 允许不同头的输出相互混合。标准多头注意力会使用全部头的输出，各头在共同损失下训练，其功能可能互补，也可能重叠。
...
为区分两条序列，记原文长度为 \\(S\sb {\mathrm{src}}\\)，当前译文前缀长度为 \\(S\sb {\mathrm{tgt}}\\)。原文经过六层 encoder，得到：
...
\\(D\\) 是 decoder 的最终状态，\\(W\sb {\mathrm{vocab}}\\) 是词表输出投影，\\(\Pi\\) 是预测概率矩阵。词表输出投影与注意力内部的 \\(W\sb {O}\\) 作用不同；前者输出词表分数，后者混合多头结果。原论文还让词表输出投影与 embedding 共享权重。
...
训练时有完整的原文和正确译文。设正确译文包含 \\(T\\) 个 token，依次记为 \\(y\sb {1},\ldots,y\sb {T}\\)。这里的下标只表示译文位置。
